...
def kpi_backtest(path_predictions, raw_data, staleness_filter, model_name, 
    ...
    if model_type == 'universal':
        ...
        predictions.columns = ['Date', 'Coin', 'prediction']
        predictions.index = [pd.to_datetime(predictions.Date), predictions.Coin]
        del(predictions['Date'])
        del(predictions['Coin'])
    else:
    ...
    K = 2 # variable that represents the top K coins you want to select in the kpi_backtest function.
