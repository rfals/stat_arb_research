...
class TrainTestSplitter:
    '''
    The TrainTestSplitter class is a utility designed to split financial time series datasets
    into training and testing sets for model training and evaluation. It is part of a larger framework 
    that supports data preprocessing, model evaluation, backtesting, and performance summary
    compilation for machine learning models.

    Attributes:
    ----------
    features: A DataFrame containing feature data for the financial time series.
    targets: A DataFrame containing target data for the financial time series.
    num_trading_days: The number of trading days considered for the split.
    sequence_length: The length of the input sequence for models like LSTM.
    forecast_horizon: The number of periods into the future to predict.

    Methods:
    -------
    split_data_into_train_and_test():
        This method takes features, targets, number of trading days, sequence length, and forecast
        horizon as input and splits the data into training and testing sets based on the specified parameters.
        It also handles overlapping cases for models that use recurrent neural networks, such as LSTM.
    '''
    def __init__(self):
...
def backtest_model(prediction_probas, k, dataprovider, forecast_horizon):
    """
    k denotes the top-k and flop-k Coins that are traded based on the 
    ...
    actual_returns.index.names = ['Date', 'Coin']
...
def compile_performance_summary(all_predictions, all_realized_returns, dataprovider, forecast_horizon, k):
    ...
    true_targets.index.names = ['Date', 'Coin']
    ...
    """ (2) Determine accuracy and log_loss for all Coins """
    ...
    """ (3) Determine accuracy and log_loss for the top/flop k Coins that are traded """
    ...
    try:
        ...
        accuracy_by_year.plot(kind='bar', title='Accuracy by year (all Coins)', 
        ...
        accuracy_by_year_top_flop_k.plot(kind='bar', title='Accuracy by year (top/flop-k Coins)',
        ...
        log_loss_by_year.plot(kind='bar', title='Log_loss by year (all Coins)', 
        ...
        log_loss_by_year_top_flop_k.plot(kind='bar', title='Log_loss by year (top/flop-k Coins)',
