...
from var_eg_stats_utils import *
...
class VectorBacktester(object):
    ...
    freq: str
        data frequency: minute, hourly, daily, weekly, monthly, quarterly, annual
    trans: str
        data transformation: diff, rdiff, cumul, and normalize
    ctf: float 
        overall crypto execution fees per trade (buy or sell) => trading fee + bid-offer + slippage (delay, broker fees, etc)
        Damodaran: Overall Costs Binance => ctf = 2% default assumption is reasonable
    rf: float
    ...
    def param(self):
        ...
        tau = 1 / (365)  # hourly data frequency
    ...
    def optimal_param_multiper(self, sd_range, slip_range, ratio, n_split=3):
        ...
        return results.sort_values(by=rat, ascending=False)
